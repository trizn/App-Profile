...
class FlickrHelper {
    ...
    func checkAuthentication(callbackURL: URL, sender: UIViewController, _ completionHandle: (()->Void?)?) {
        self.completeAuthOp = FlickrKit.shared().completeAuth(with: callbackURL, completion: {
            ...
            DispatchQueue.main.async(execute: { () -> Void in
                if error == nil {
                    User.share().getProfileInfo(userName: userName, fullName: fullName, userId: userID)
                } else {
...
//                HelperAlert.showAlert(sender: sender, title: "Error", message: message)
